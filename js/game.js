...
//* LET'S GOO!
game.init();
// cet event est appelé une fois et détectera le changement de page pour activer une pause si le jeu est lancé
...
- grid.js permet de générer les positions de la dessiner et de placer une pièce arrivée en bout de parcours
- piece.js permet de générer une pièce aléatoirement et de la dessiner puis de la redessiner tout au long de son parcours
- data.js regroupe un certain nombre de données spécifiques pour faciliter leur pilotage
- scoring.js permet la gestion du score de la partie courante et l'interaction avec le local storage pour stocker, update et ou récupérer le meilleure score
...
- running: booléen qui détermine si la partie est en cours à (true) ou non à (false)
- over,  booléen qui détermine si la partie est terminée ou non
- interval: qui va recevoir le setInterval du jeu (permet de gérer le parcous d'une pièce jusqu'à son positionnement)
- speed: définit la vitesse de déplacement de la pièce (set interval)
- end_positions: regroupe les positions de la dernière ligne de la grille
- button: reçoit l'élément du DOM qui permet de lancé la partie lors du clique sur cet élément
...
- init() permet d'initialiser une partie, on va y appeler plusieurs méthodes et effectuer différentes actions:
    ...
    - génération des valeurs de la grille et de leurs attributs
    - dessin de la grille dans la balise canvas à l'aide des valeurs générées juste avant
    ...
    - ajout de l'event click sur le bouton de lancement de partie. Au click sur ce bouton:
        ...
        - on clear l'interval précédant (pour le cas ou la partie a été relancée)
...
- launch_game(), permet de lancer la partie, détail des instructions:
...
- on_move(), permet de gérer tout le parcours de la pièce courante:
    ...
    - ensuite on a 3 bloques de vérifications qui vont changer ou non notre flag:
        ...
        3ème bloque: check si une des position de la pièce courante se trouve sur la case juste avant une case occupée par une pièce déjà placée:
            ...
            - pour vérifier si il y a un match on va anticiper la valeur qu'aura la positions de la pièce courante lors de son prochain déplacement on peut le traduire comme ceci:
                - si la position courante de la pièce + une case vers le bas match avec la position de la grille et que la position de la grille est occupée par une pièce qui est déjà placée
